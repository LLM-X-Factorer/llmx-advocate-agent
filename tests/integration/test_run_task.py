...
@pytest.mark.asyncio
async def test_run_task_progresses_through_p3_then_pauses_at_stub(db_session, engine):
    """P1..P3 all implemented; P4 still a stub. Pauses at P4."""
    ...
    assert outcome.final_phase == PhaseId.P4

    runs = await repo.list_phase_runs(db_session, task.id)
    by_phase = {r.phase_id: r for r in runs}
    for pid in (PhaseId.P1, PhaseId.P1_5, PhaseId.P2, PhaseId.P2_5, PhaseId.P2_6, PhaseId.P3):
        assert by_phase[pid].status.value == "passed", f"{pid} should pass"
    assert by_phase[PhaseId.P4].status.value == "error"
